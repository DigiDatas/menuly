...
const safeId = row.A ? String(row.A).trim() : 'item-' + Math.random().toString(36).substr(2, 9);
                ...
                const brandLogo = rawBranding.smartLogo || 'https://raw.githubusercontent.com/DigiDatas/ShawarMax-Menu/main/logo.png';
                ...
                const item = {
                    ...
                    imgUrls: (() => {
                        ...
                        if (row.K) {
                            urls = String(row.K).split(',').map(u => {
                                ...
                                if (!clean.startsWith('http')) return `https://raw.githubusercontent.com/DigiDatas/ShawarMax-Menu/main/images/ItemImages/${clean}${clean.includes('?') ? '' : v}`;
                                return clean;
                            }).filter(u => u !== null);
                        } else {
                    ...
                    img: (() => {
                        if (row.K) {
                            ...
                            return first.startsWith('http') ? first : `https://raw.githubusercontent.com/DigiDatas/ShawarMax-Menu/main/images/ItemImages/${first}`;
                        }
                        return brandLogo;
                    })(),
                    ...
                    if (bannerArray && Array.isArray(bannerArray)) {
                        ...
                        if (sectionBannerStr && sectionBannerStr.trim() !== "") {
                            const bannerUrls = sectionBannerStr.split(',').map(u => u.trim()).filter(Boolean).map(u => {
                                if (!u.startsWith('http')) return `https://raw.githubusercontent.com/DigiDatas/ShawarMax-Menu/main/images/banner/${u}`;
                                return u;
                            });
    ...
    updateQuery = supabaseClient.from('menu_items').update({ stock_quantity: newStock }).eq('restaurant_id', restaurantId).eq('id', cartItem.id); // 🔒 Scoped
        ...
        function openCategoryPage(title, isHighlight) {
            ...
            items.forEach((item, index) => {
                ...
                let imagesHtml = '';
                urls.forEach(url => {
                    imagesHtml += `<img src="${url}" loading="lazy" alt="Image" class="w-full h-full object-cover shrink-0 snap-center group-hover:scale-105 transition-transform duration-500" onerror="if(this.src.includes('.jpg')) { this.src=this.src.replace('.jpg', '.png'); } else if(this.src.includes('.png')) { this.src=this.src.replace('.png', '.jpeg'); } else { if(this.parentElement && this.parentElement.children.length > 1) { this.remove(); } else { this.onerror=null; this.classList.replace('object-cover', 'object-contain'); this.classList.add('p-4'); this.src='${rawBranding.smartLogo || 'https://raw.githubusercontent.com/DigiDatas/ShawarMax-Menu/main/logo.png'}'; } }">`;
                });
      ...
      // --- NEW: Recipe Expand Logic ---
        ...
        function gatherFlashCards() {
            ...
            if (window.liveFlashcards && window.liveFlashcards.length > 0) {
                ...
                const baseUrl = 'https://raw.githubusercontent.com/DigiDatas/ShawarMax-Menu/main/images/FlashCard/FlashCard';
                for (let i = 1; i <= 15; i++) {
...
// --- NEW: BRAND PROFILE MODAL LOGIC ---
        ...
        function openBrandModal() {
            ...
            img.src = rawBranding.smartLogo || 'https://raw.githubusercontent.com/DigiDatas/ShawarMax-Menu/main/logo.png'; 
